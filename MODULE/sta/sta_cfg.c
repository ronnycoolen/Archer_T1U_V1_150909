...
/*                kfree(pStatistics); */
	...
	case RT_OID_802_11_QUERY_TXBF_TABLE:
		if (pAd->chipCap.FlgHwTxBfCap)
		{
			...
			//pMacTab = (RT_802_11_TXBF_TABLE *)kmalloc(sizeof(RT_802_11_TXBF_TABLE), GFP_ATOMIC);
		...
		if (pAd->MacTab.Size > 0)
		{
			...
			for (i=1; i<MAX_LEN_OF_MAC_TABLE; i++)
			{
				...
				if (IS_ENTRY_CLIENT(pEntry) && pEntry->Sst==SST_ASSOC)
				{
					...
					UINT32 lastTxRate = pAd->LastTxRate;

#ifdef RT65xx
					if (IS_MT76x0(pAd)) {
						sprintf(extra+strlen(extra), "Last TX Rate                    = MCS %d, %s, %cGI, %s%s\n",
							lastTxRate & 0x7F,
							bw[((lastTxRate>>7) & 0x3)],
