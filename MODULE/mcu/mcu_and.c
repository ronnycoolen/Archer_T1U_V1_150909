...
NDIS_STATUS andes_usb_loadfw(RTMP_ADAPTER *ad)
{
	...
	if (cap->IsComboChip)
	{
		for(loop = 0; loop < 6; loop++)
		{
			...
			if ( (mac_value & 0x1) == 0x01)
			{
				// Continue to check COM_REG0[0]
				break;
			}
			else if ( (mac_value & 0x1) == 0x00)
			{
	...
	while (1)
	{
		...
		if (sent_len > 0)
		{
			...
			RTUSBReadMACRegister(ad, 0x234, &mac_value);
			while (!(mac_value & 0x80000000)) {
				DBGPRINT(RT_DEBUG_ERROR, ("DDone is not ready, wait for 2ms\n"));
				RtmpOsMsDelay(2);
				if (loop > 100) {
					DBGPRINT(RT_DEBUG_ERROR, ("DDone is still not ready, wait for timeout\n"));
					break;
				}
				RTUSBReadMACRegister(ad, 0x234, &mac_value);
				loop++;
